...
exports.mystore_menu = function(request, response, name) {
  db.query('select store_number, permission from store where store_name = ? and id = ?', [name, request.session.ID], function(error, storeN){
    ...
    db.query('select * from menu where store_number =?', [storeNumber], function(error, menuList){
      db.query('select * from seat where store_number = ? ORDER BY date DESC LIMIT 1', [storeNumber], function(error, seat){
        response.render('mystore.html', {data:menuList, name:name, permission:permission, storeNumber:storeNumber, seat:seat});
      });
    })
  })
}
...
exports.menu_update = function(request, response, post, name) {
  db.query('update menu set menu_name=?, price=? where menu_id=? ', [post.menu_name, post.price, post.menu_id], function(error, result){
  if (error) {
    throw error;
  }
    response.redirect('/user')
...
exports.menu_delete = function(request, response, post, name) {
  db.query('delete from menu where menu_id=? ', [post.menu_id], function(error, result){
  if (error) {
    throw error;
  }
    response.redirect('/user')
